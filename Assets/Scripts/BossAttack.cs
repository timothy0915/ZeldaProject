...
public class BossAttack : MonoBehaviour
{
    ...
    public float attackCooldown = 3f;     // 攻擊間隔時間（攻擊冷卻時間）
    public float magicattackCooldown = 3f;     // 攻擊間隔時間（攻擊冷卻時間）
    public float knockbackForce = 5f;     // 攻擊時施加在玩家身上的擊退力度
    public float attackDamage = 3f;      // 攻擊造成的傷害量
    public float magicattackDamage = 5f;      // 攻擊造成的傷害量
    ...
    private void TryAttackPlayer()
    {
           ...
           
        {
            ...
            if (animator != null)
                if (Random.value < 0.5f) //50% 觸發ATK 50%觸發ATK1
                {
            ...
            Invoke("Attaking", 0.5f); // 延遲 0.5 秒執行攻擊
        }
    }
    void Attaking()
    {
